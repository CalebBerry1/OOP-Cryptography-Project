...
void mainXOR() {
    ...
    for (char c : targetOriginal) {
        ...
        // PHASE 1: converts the character into a byte; adds the byte to targetBinary, which is the target message but in binary
        // targetBinary is a vector of bitsets, with each bitset being 1 character of the message
        targetBinary.push_back(makeCharToBitset(c));
