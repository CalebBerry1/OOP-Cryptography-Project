...
vector<bitset<8>> XorStreamCipher::toBinary(const string& input) {
    vector<bitset<8>> binary;
    for (char c : input) {
        binary.push_back(bitset<8>(c));
    }
    return binary;
}

vector<bitset<8>> XorStreamCipher::generateKey(int length) {
    vector<bitset<8>> key;
    for (int i = 0; i < length; i++) {
        key.push_back(bitset<8>(rand() % 256));
    }
    return key;
}

vector<bitset<8>> XorStreamCipher::applyXor(
    const vector<bitset<8>>& input,
    const vector<bitset<8>>& key)
{
    vector<bitset<8>> result = input;
...
string XorStreamCipher::fromBinary(const vector<bitset<8>>& binary) {
    string result;
    for (const bitset<8>& byte : binary) {
...
CryptoResult XorStreamCipher::run(const CryptoRequest& req, ICryptoStepSink& sink) {
    try {
        ...
        string input = req.input.toStdString();
        ...
        string resultStr = fromBinary(resultBinary);
...
// returns a pair of (leftHalf, rightHalf)
pair<string, string> XorStreamCipher::run(const string& block1, const string& block2) {
    try {
        qDebug() << "[XorStreamCipher] run() called";
        string leftHalf = block1;
        string rightHalf = block2;
        ...
        string resultLeft = fromBinary(encryptedLeft);
        string resultRight = fromBinary(binaryRight);
        //QByteArray output = QByteArray::fromStdString(resultStr);
